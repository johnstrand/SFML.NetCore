...
public class RenderWindow : Window.Window, IRenderTarget
{
    ...
    {
        _myDefaultView = new View(sfRenderWindow_getDefaultView(CPointer));
        GC.SuppressFinalize(_myDefaultView);
    }

    private View _myDefaultView;
    ...
    [DllImport(CSFML.graphics, CallingConvention = CallingConvention.Cdecl), SuppressUnmanagedCodeSecurity]
    private static extern unsafe void sfRenderWindow_drawPrimitives(IntPtr cPointer, Vertex* vertexPtr, uint vertexCount, PrimitiveType type, ref RenderStates.MarshalData renderStates);
